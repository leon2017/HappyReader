...
import com.leonwang.app.chinashop.db.dao.WeatherDao;
...
import com.leonwang.app.chinashop.utils.DecodeUtil;
import com.leonwang.app.chinashop.utils.LogUtils;
import com.leonwang.app.chinashop.utils.NetworkUtil;
...
//                        weekForeCasts.remove(weekForeCasts.size() - 1);
                    ...
                    if (realWeather == null || aqi == null || weekForeCasts.size() == 0 || hourForeCasts.size() == 0 || zhishus.size() == 0 || !useLocal)
                        if (NetworkUtil.checkNetwork(_context))
                            ...
                            sendEmptyMessage(handler, ConstantUtils.MSG_ERROR);
                    else {
    ...
    public void loadWeatherFromNet(final String areaID, final Handler handler) {
        new Thread(new Runnable() {
            ...
            public void run() {
                ...
                String url = String.format(ConstantUtils.URL_WEATHER_2345, city.getAreaId());
                String urlFlyme = String.format(ConstantUtils.URL_FORECAST_FLYME, city.getWeatherId());
                try {
                    ...
                    Log.d("flymeresponse", flymeresponse.toString());
                    LogUtils.d(response.toString());
                    LogUtils.d(flymeresponse.toString());
...
//                    }
                    ...
                    for (int i = 0; i < jsonArrayzss.size(); i++) {
                        ...
                        zhishu.setAreaid(areaID);
                        zhishu.setName(jsonObject.getString("name"));
                        if (ConstantUtils.ZHISHU.get(zhishu.getName()) == null)
                            continue;
